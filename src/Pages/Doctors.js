...
function Doctors() {
    ...
    useEffect(()=>{
        axios.get(`https://goel-backend.onrender.com/get-doctors`)
        .then((res)=> {
  ...
  const submitBlog = () => {
    ...
    if(!name || !imageUpload || !speciality){
        alert("Please enter name, speciality  and image")
    }
    ...
    const imageRef = ref(storage, `doctor/${finalId}`)
    uploadBytes(imageRef, imageUpload).then(()=>{

      getDownloadURL(ref(storage, `doctor/${finalId}`))
  .then((url) => {
    axios.post('https://goel-backend.onrender.com/add-doctor', {name, speciality, image: url, facebook, instagram: insta,linkedin: linkedIn})
    .then((res)=>{
    ...
    <div className="collapse navbar-collapse" id="navbarSupportedContent">
      <ul className="navbar-nav mr-auto">
        ...
        <li className="nav-item">
          <Link className="nav-link active" to="/doctors">Our Doctors</Link>
        </li>
        <li className="nav-item">
  ...
  <div>
      ...
      >
        ...
        <div style={{display: 'flex', flexDirection: 'column', width: '100%', alignItems: 'center'}}>
          <h1>Add Doctor</h1>
          <input type='text' className='myInputClass' name='name' placeholder='Enter The Name of Doctor' value={name} onChange={(e)=> {setName(e.target.value)}}/>
          <input type='text' className='myInputClass' name='speciality' placeholder='Enter The Speciality of Doctor' value={speciality} onChange={(e)=> {setSpeciality(e.target.value)}}/>

        <h4 style={{marginTop: '20px'}}>Choose Image</h4>
          <input type='file' className='myInputClass' onChange={(e)=>{setImageUpload(e.target.files[0])}}/>

          <input type='text' className='myInputClass' name='facebook' placeholder='Enter Facebook ID Link Here' value={facebook} onChange={(e)=> {setFacebook(e.target.value)}}/>

          <input type='text' className='myInputClass' name='linkedIn' placeholder='Enter LinkedIn ID Link Here' value={linkedIn} onChange={(e)=> {setLinkedIn(e.target.value)}}/>
          
          <input type='text' className='myInputClass' name='Instagram' placeholder='Enter Instagram ID Link Here' value={insta} onChange={(e)=> {setInsta(e.target.value)}}/>
...

{
   ...
   ?     
        testData.map((obj)=> {
            return(
                ...
                <i className="fa-solid fa-trash" id='testDel' style={{position: 'absolute', top: '-70px'}} onClick={()=>{
                  axios.delete('https://goel-backend.onrender.com/delete-doctor', {_id: obj._id})
                  .then(()=>{
      ...
      <div className="card-body">
        ...
        <p className="card-text">{`Speciality : ${obj.speciality}`}</p>
      </div>
    </div>
    </>
            )
...
<>
        ...
        <h1 style={{paddingLeft: '20px'}}>No Doctors Found</h1>
        </>
